...
| `term-sessions.js` | Leaf: localStorage CRUD for per-worktree console descriptors (`loadSessions`/`getSession`/`setSession`/`removeSession`); key `localpm.termSessions`; one `{ sessionId, kind }` per path. No DOM, no app imports. |
...
| `keynav/keynav.js` | Global capture-phase keydown scaffold: `initKeynav()` (desktop-only). NAV mode: `Enter`/`i` focuses terminal + switches to WRITING; `gt`/`gT` move to next/prev project; `↑`/`↓` step through every worktree across the whole tree, crossing project boundaries + auto-expanding (arrows prevent page scroll). WRITING mode: double-Esc blurs terminal + returns to NAV; lone Esc propagates to xterm. |
| `keynav/traversal.js` | Pure index math over the grouped tree: `nextProject`/`prevProject` (per-project) and `nextInTree`/`prevInTree` (flattened worktree list across all projects). Boundaries wrap; null/unknown selection starts at an edge. Empty tree → null. No DOM. Imports `grouping.js` only. Unit-tested. |
| `keynav/fuzzy.js` | Pure vanilla subsequence fuzzy matcher: `score(query, target)` → 0 (no match) or positive (higher = better). Contiguous + word-boundary bonuses. Case-insensitive. No deps, no DOM. Unit-tested. |
| `keynav/mru.js` | Leaf: localStorage CRUD for MRU worktree order (`loadMru`/`recordMru`). Single key `localpm.navMru`. Tolerant reads/writes (mirrors `term-macros.js`). No DOM, no app imports. Unit-tested. |
| `keynav/palette.js` | Quick-nav palette overlay (`openPalette`/`closePalette`/`refreshPaletteIfOpen`). Mounted on `document.body` outside the re-rendered subtree (survives 2s poll). Fuzzy-filters `project / branch` rows; empty query shows MRU-first. Running markers, `↑`/`↓` highlight, `Enter`/click jumps via `selectItem()`. `Esc` closes only (no mode change). Imports `fuzzy.js`, `mru.js`, `selection.js`, `grouping.js`, `app-events.js`. |
