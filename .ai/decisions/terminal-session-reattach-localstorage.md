...
**Reconnect is a one-shot bootstrap at first tick only:** `reconnectActiveWorktree` is called once in `main.js` `tick()` after the first successful state fetch + `render()`, using the selection resolved at that moment. A module-level `reattached` guard prevents it from refiring on the 2 s poll. Only the worktree active at that first tick is reconnected; this scopes the eager reattach to the page-load worktree and avoids reconnecting every worktree's session on load. Navigating to a *different* worktree afterward does **not** reattach — the normal `+ Shell` / `+ Claude` open path calls `openTerminal` with a fresh `newSessionId()` and never consults the store, so it spawns a new console. (Reattaching on later navigation was not in scope; the saved descriptor only drives the load-time bootstrap.)
